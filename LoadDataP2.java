...
public class LoadDataP2 {
	...
	public void search(String[] commands, String[] inputs) {
		...
		if (commands[0].equals("-C")) {
			...
			while (iter.hasNext()) {
				...
				int date = Integer.parseInt(changeDateFormat(pair.getK()));
				...
				while (streak) {
					...
					BinaryNode<Pair<String, String>, State> node = 
							bstNameDate.find(new Pair<String,String>(pair.getT(), changeDate((date + newCount) + "")));
				...
				if (count >= 7) {
					...
					//prints date range
					System.out.println(pair.getK() + " - " + changeDate(tempDate + ""));
				}
				while (count != 1) {
					iter.next();
					count--;
		...
		if (commands[0].equals("-T")) {
			...
			while (iter.hasNext()) {
				...
				int newCount = 0;
				Pair<String, String> pair = iter.next();
				int month = Integer.parseInt(pair.getK().substring(0, 2));
				...
				while (dayCounter < range) {
					...
					BinaryNode<Pair<String, String>, State> node = 
							bstNameDate.find(new Pair<String,String>(pair.getT(), changeDate((dateNum + newCount) + "")));
					if (node != null) {
		...
		while (commands[i] != null) {
			...
			if (commands[i].equals("-D")) {
			    if (!isValidDate(inputs[i])) {
			        System.out.println("The date " + inputs[i] + " is not valid");
			        return;
			    }
				isD = true;      
		...
		if (isS) {
		    if (!isValidState(newInputs[0])) {
		        System.out.println("The state of " + newInputs[0] + " does not exist");
		        return;
		    }
			java.util.Iterator<Pair<String, String>> iter = bstNameDate.iterator();
			while (iter.hasNext()) {
				...
				if (!name.isEmpty()) {
					if (name.length() != 2) {
						name = changeAbbrev(name.toLowerCase());
					}
		...
		if (isQ) {
		    if (!isValidLetter(newInputs[1])) {
		        System.out.println(newInputs[1] + " is not a valid quality grade");
		        return;
		    }
			if (isS) {
		...
		if (isS || isQ) {
			if (isN) {
				...
				while (iter.hasNext()) {
					Pair<String, String> pair = iter.next();
					if (!changeDateFormat(pair.getK()).equals(date)) {
						BinaryNode<Pair<String, String>, State> node = tempNameStateBST.find(pair);
						tempNameStateBST.remove(pair, node.getValue());
					}
				}
		    ...
		    String recentDate;
			if (isD) {
			    recentDate = changeDateFormat(newInputs[3]);
			    lastDateForPrinting = changeDate(recentDate);
			}
			else {
			...
			if (isN) {
			    String firstDate = getFirstDate(recentDate, Integer.parseInt(newInputs[2]));
			    firstDateForPrinting = changeDate(firstDate);
			    ...
			    while (iter.hasNext()) {
			        Pair<String, String> pair = iter.next();
			        if (pair.getT().equals(currState)) {
			            continue;
			        }
			        int dateNum = Integer.parseInt(firstDate);
			        int month = Integer.parseInt(pair.getK().substring(0, 2));
			        int dayCounter = 0;
			        int newCount = 0;
			        int range = Integer.parseInt(newInputs[2]);
			        ...
			        while (dayCounter < range) {
			            ...
			            Pair<String, String> tempPair = new Pair<String, String>(
                            pair.getT(), changeDate((dateNum + dayCounter) + ""));
			            BinaryNode<Pair<String, String>, State> node = bstNameDate.find(tempPair);
			            if (node != null) {
			                tempNameStateBST.insert(tempPair, node.getValue());
			            }
			            dayCounter++;
			            newCount++;
			        }
			    ...
			    while (iter.hasNext()) {
                    Pair<String, String> pair = iter.next();
                    String iterDateString = pair.getT();
                    if (iterDateString.equals(recentDate)) {
                        BinaryNode<Pair<String, String>, State> node = bstDateName.find(pair);
                        tempNameStateBST.insert(pair, node.getValue());
                    }
                }
			}
		...
		if (isD || isS || isQ || isN) {
			...
			while (iter.hasNext()) {
				...
				System.out.println(state.getState() + "\t" + state.getDate() + "\t" + changeNumbers(state.getPositives()) + "\t\t" +
			...
			if (isS) {
				System.out.print(" for state " + changeAbbrev(newInputs[0].toLowerCase()).toUpperCase()); 
			}
			if (isQ) {
			...
			if (isD) {
				if (!isN) {
					System.out.print(" on date " + lastDateForPrinting);
				}
				else {
				...
				System.out.print(" from " + firstDateForPrinting + " to " + 
						lastDateForPrinting);
			}
			System.out.println();
		}  
	...
	/**
	 * Used is -S is called but with no state
	 */
	private void searchJustState() {
	    java.util.Iterator<Pair<String, String>> iter = bstNameDate.iterator();
	    ...
	    System.out.println(bstNameDate.getSize() + " records have been printed");
	}
	...
	private void searchNoParam() {
		...
		while (iter.hasNext()) {
			Pair<String, String> pair = iter.next();
			String iterDate = changeDateFormat(pair.getT());
		...
		while (counter < i) {
			System.out.println(stateArr1[counter].getState() + "\t" + date + "\t" + changeNumbers(stateArr1[counter].getPositives()) + "\t\t" +
	...
	private String recentDate() {
		...
		highestDate = changeDateFormat(highestDate);
 
		while (iter.hasNext()) {
			String newDate = changeDateFormat(iter.next().getT());
	...
	private String changeDateFormat(String highestDate) {
	    StringBuilder sb = new StringBuilder();
        sb.append(highestDate.substring(highestDate.length() - 4, highestDate.length()));
        sb.append(highestDate.substring(0, 2));
        sb.append(highestDate.substring(3, 5));               
        return sb.toString();
	}
	...
	/**
	 * Removes all the data lower than or equal to the quality grade in the parameter
	 * 
	 * @param grade - The grade in the parameter
	 */
	public void remove(String grade) {
        ...
        while (iter.hasNext()) {
            Pair<String, Pair<String, String>> pair = iter.next();
            if (grade.equals(pair.getT()) || isGradeBetter(grade, pair.getT())) {
                BinaryNode<Pair<String, Pair<String, String>>, State> node = bstGrade.find(pair);
                bstGrade.remove(node.getKey(), node.getValue());;
                bstNameDate.remove(node.getKey().getK(), node.getValue());
                Pair<String, String> dateName = new Pair<String, String>(node.getKey().getK().getK(), node.getKey().getK().getT());
                bstDateName.remove(dateName, node.getValue());
                counter++;
            }
        }
        System.out.println(counter + " records with quality grade lower or equal to " + grade + " have been removed");
	}
	
	/**
     * Returns whether the quality grade is valid
     * 
     * @param str The quality grade
     * @return True is the quality grade is valid
     *             False if the quality grade is no valid.
     */
    private boolean isValidLetter(String str) {
        return (str.equals("A") || str.equals("A+") || str.equals("B") ||
            str.equals("B+") || str.equals("C") || str.equals("C+") || 
            str.equals("D") || str.equals("D+") || str.equals("F")); 
    }
    
    /**
     * Checks whether input is a valid state
     * 
     * @param str The state in question
     * @return True if the state is valid
     *              False if the state is not valid
     */
    private boolean isValidState(String str) {
        boolean bool = false;
        for (int i = 0; i < state_abvs.length; i++) {
            String lower = str.toLowerCase();
            String upper = str.toUpperCase();
            
            if (lower.equals(stateNames[i]) || upper.equals(state_abvs[i])) {
                bool = true;
            }
        }
    ...
    private boolean isValidDate(String date) {
        return ((date.length() == 10) && (Integer.parseInt(date.substring(0, 2)) <= 12)
